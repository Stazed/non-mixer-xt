...
Controller_Module::connect_to( Port *p )
{
    ...
    if(LV2_PORT(p))
    {
        ...
        LV2_PORT(&control_output[0])->ScalePoints = LV2_PORT(p)->ScalePoints;
    }
#endif
    clear ( );
    ...
    else if ( p->hints.type == Module::Port::Hints::LV2_INTEGER_ENUMERATION )
    {
        ...
        for ( unsigned count = 0; count < LV2_PORT(p)->ScalePoints.size ( ); ++count )
        {
            o->add ( LV2_PORT(p)->ScalePoints[count].Label.c_str ( ) );
        }
        ...
        for ( unsigned i = 0; i < LV2_PORT(p)->ScalePoints.size ( ); ++i )
        {
            if ( (int) LV2_PORT(p)->ScalePoints[i].Value == (int) ( p->control_value ( ) + .5 ) ) // .5 for float rounding
            {
                menu_location = i;
                break;
            }
...
Controller_Module::cb_handle( Fl_Widget *w )
{
    ...
    else if ( type ( ) == CHOICE )
    {
        ...
        control_value = LV2_PORT(&control_output[0])->ScalePoints[menu_location].Value;
    }
#endif
    else
...
Controller_Module::handle_control_changed( Port *p )
{
    ...
    else
    {
        ...
        else if ( type ( ) == CHOICE )
        {
            ...
            for ( unsigned i = 0; i < LV2_PORT(p)->ScalePoints.size ( ); ++i )
            {
                if ( (int) LV2_PORT(p)->ScalePoints[i].Value == (int) ( control_value + .5 ) ) // .5 for float rounding
                {
                    menu_location = i;
                    break;
                }
